...
```bash
rg "def (encode|serialize|compress|pack|to_json|to_bytes)" --type py
rg "def (decode|deserialize|decompress|unpack|from_json|from_bytes)" --type py
# Compare the two lists — missing inverses are refactoring opportunities
```
...
# AFTER - structured representation enables roundtrip
@dataclass
class Query:
    table: str
    filters: dict
