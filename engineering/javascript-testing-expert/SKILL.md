...
description: Companion to `tdd` and `integration-testing` for idiomatic JavaScript/TypeScript test code. Use when you already know you need tests in JS/TS and want the right patterns, tooling, and structure with vitest, fast-check, testing-library, msw, or browser/snapshot testing. Does not cover black-box e2e testing.
...
**❌ Don't** split simple and advanced tests into separate sections; put them in a useful reading order
...
**❌ Don't** use one large `prepare` function for every test; give setup helpers specific names and split unrelated setup
