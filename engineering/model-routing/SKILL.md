---
name: model-routing
description: Route delegated model portfolios across Pi/API and CLI-only access. Use when selecting models for delegated roles, structuring multi-family review and synthesis, or adding family diversity to frontend work, alternative implementations, and adversarial review.
---

# Model Routing

Route a **portfolio**: an OpenAI primary lane supplies the capability baseline, a GLM lane supplies general family diversity, and a Grok lane supplies adversarial diversity. Plan every role before delegation.
...
## 4. Add the Grok adversarial lane

Route Grok roles to `cursor-grok-4.6-xhigh` through Cursor's `agent` CLI. Grok is CLI-only in this environment: launch a separate process and pass its report to the delegating or Synthesis agent. Keep Pi and workflow model fields for Pi-accessible routes such as OpenAI and GLM.

Before each launch, run `agent --help` and `agent --list-models`; verify the current Grok 4.6 Extra High identifier and derive flags from installed help. With the current interface, a read-only repository review uses:
...
- adversarial review triggers, especially debugging, race, concurrency, resource-lifetime, and merge-safety analysis;
- a final challenge to a chosen approach when correlated assumptions across the OpenAI and GLM lanes would be costly.

Keep the launch read-only unless the task explicitly authorizes probes or edits. When the user requests a probe-capable Grok debug review in Herdr, invoke `$herdr-grok-review` instead of recreating its launch and checkout-preservation contract.
...
This step is complete when every Adversarial review and high-stakes task has a captured Grok report or a concrete CLI availability reason, and every completed multi-family review has a Sol `medium` Synthesis agent or a recorded fallback.
